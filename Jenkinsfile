...
node('master') {
  ...
  withEnv(["SLACK_HOOK=${SLACK_HOOK}", "DEV_CHANNEL=${DEV_CHANNEL}"]){
    stage('Build'){
      ...
      try {
        echo "Building: mem-admin"
        openshiftBuild bldCfg: 'mem-admin', showBuildLogs: 'true'
        echo "Build done"
        ...
        IMAGE_HASH = sh (
        script: """oc get istag mem-admin:latest -o template --template=\"{{.image.dockerImageReference}}\"|awk -F \":\" \'{print \$3}\'""",
        returnStdout: true).trim()
        echo ">> IMAGE_HASH: ${IMAGE_HASH}"

        openshiftTag destStream: 'mem-admin', verbose: 'true', destTag: "${IMAGE_HASH}", srcStream: 'mem-admin', srcTag: 'latest'
        echo "Tagging done"
      } catch (error) {
        notifySlack(
          "The latest mem-admin build seems to have broken\n'${error.message}'",
          env.SLACK_HOOK,
          env.DEV_CHANNEL,
          []
        )
  ...
  withEnv(["SLACK_HOOK=${SLACK_HOOK}", "DEPLOY_CHANNEL=${DEPLOY_CHANNEL}", "QA_CHANNEL=${QA_CHANNEL}"]){
    stage('Deploy to Test'){
      try {
        ...
        openshiftTag destStream: 'mem-admin', verbose: 'true', destTag: 'test', srcStream: 'mem-admin', srcTag: 'latest'
        sleep 5
        openshiftVerifyDeployment depCfg: 'mem-mmt-test', namespace: 'mem-mmt-test', replicaCount: 1, verbose: 'false', verifyReplicaCount: 'false', waitTime: 600000
        echo ">>>> Deployment Complete"

        notifySlack(
          "A new version of mem-admin is now in Test. \n Changes: \n ${CHANGELOG}",
          env.SLACK_HOOK,
          env.DEPLOY_CHANNEL,
          []
        )
        ...
        notifySlack(
          "A new version of mem-admin is now in Test and ready for QA. \n Changes to test: \n ${CHANGELOG}",
          env.SLACK_HOOK,
          env.QA_CHANNEL,
          []
        )
        ...
        notifySlack(
          "The latest deployment of mem-admin to Test seems to have failed\n'${error.message}'",
          env.SLACK_HOOK,
          env.DEPLOY_CHANNEL,
          []
        )
