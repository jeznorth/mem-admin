...
angular
  .module('invitations', [])
  .controller('EditInvitationController', ['$scope', '$state', '$uibModal', 'Authentication', 'NgTableParams', '_', 'CommunicationModel', 'project', 'communication', 'mode', function EditInvitationController($scope, $state, $uibModal, Authentication, NgTableParams, _, CommunicationModel, project, communication, mode) {
    ...
    var $locationChangeStartUnbind = $scope.$on('$stateChangeStart', function (event, toState) {
      // no check
    ...
    var doSend = function(communication) {
      ...
      modalDocView.result.then(function () {
        ...
        // swallow rejecte dpromise error
      });
    };
