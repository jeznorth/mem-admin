...
angular.module('documents')
  ...
  .directive('documentMgr', ['_', 'AlertService', 'ConfirmService', 'FolderModel', function (_, AlertService, ConfirmService, FolderModel) {
    return {
      restrict: 'E',
      scope: {
        project: '=',
      ...
      controller: function ($scope, $filter, $log, _, moment, Authentication, DocumentMgrService, MinioService, TreeModel, ProjectModel, Document, CollectionModel) {
          ...
          var filePromises = _.map(files, function(f) {
            return Document.publish(f)
              .then(function(/* result */) {
                ...
                AlertService.error('File "' + f.displayName + '" could not be published.', 4000);
              });
          });
          ...
          var filePromises = _.map(files, function(f) {
            return Document.unpublish(f)
              .then(function(/* result */) {
                ...
                AlertService.error('File "' + f.displayName + '" could not be unpublished.', 4000);
              });
          });
          var dirPromises = _.map(dirs, function(d) {
